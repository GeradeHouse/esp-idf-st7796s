...
void lcdInit(TFT_t *dev, int width, int height, int offsetx, int offsety) {
    ...
    dev->_width = width;       // Display width
    dev->_height = height;     // Display height
    dev->_offsetx = offsetx;   // X-axis offset
    dev->_offsety = offsety;   // Y-axis offset
    dev->_font_direction = DIRECTION0; // Text direction
    dev->_font_fill = false;            // Disable font fill
    dev->_font_underline = false;       // Disable font underline

    ESP_LOGI(TAG, "Initializing ST7796S LCD");

    // Check if RESET pin is used
    if (dev->_reset >= 0) {
        // Perform a hardware reset
        gpio_set_level(dev->_reset, 0); // Set RESET pin low to reset the display
        delayMS(20);                    // Wait for 20 milliseconds
        gpio_set_level(dev->_reset, 1); // Set RESET pin high to finish reset
        delayMS(120);                   // Wait for display to stabilize
    }

    // Send software reset command to the display
    ESP_LOGI(TAG, "Sending Software Reset");
    spi_master_write_command(dev, 0x01); // Software Reset command
    delayMS(150);                        // Wait for reset to complete

    // Exit sleep mode to turn on the display
    ESP_LOGI(TAG, "Exiting Sleep Mode");
    spi_master_write_command(dev, 0x11); // Sleep Out command
    delayMS(120);                        // Wait for the display to wake up

    // Set the memory data access control (orientation of the display)
    ESP_LOGI(TAG, "Setting Memory Data Access Control");
    spi_master_write_command(dev, 0x36); // Memory Data Access Control command
    spi_master_write_data_byte(dev, 0x48); // Set the display orientation (0x48 for landscape)
    delayMS(10);

    // Set the pixel format to 16 bits per pixel
    ESP_LOGI(TAG, "Setting Interface Pixel Format");
    spi_master_write_command(dev, 0x3A); // Interface Pixel Format command
    spi_master_write_data_byte(dev, 0x55); // 16 bits per pixel (RGB565 format)
    delayMS(10);

    // Configure porch settings (non-visible periods at the start/end of frames)
    ESP_LOGI(TAG, "Setting Porch Control");
    spi_master_write_command(dev, 0xB2); // Porch Setting command
    spi_master_write_data_byte(dev, 0x0C); // Front porch setting
    spi_master_write_data_byte(dev, 0x0C); // Back porch setting
    spi_master_write_data_byte(dev, 0x00); // Shift line
    spi_master_write_data_byte(dev, 0x33); // Additional settings
    spi_master_write_data_byte(dev, 0x33); // Additional settings
    delayMS(10);

    // Set the VCOM voltage (controls display flickering)
    ESP_LOGI(TAG, "Setting VCOM");
    spi_master_write_command(dev, 0xBB); // VCOM Setting command
    spi_master_write_data_byte(dev, 0x35); // VCOM voltage level
    delayMS(10);

    // Configure the LCD control settings
    ESP_LOGI(TAG, "Setting LCM Control");
    spi_master_write_command(dev, 0xC0); // LCM Control command
    spi_master_write_data_byte(dev, 0x2C); // Default setting
    delayMS(10);

    // Enable VDV and VRH command writing
    ESP_LOGI(TAG, "Enabling VDV and VRH Commands");
    spi_master_write_command(dev, 0xC2); // VDV and VRH Command Enable
    spi_master_write_data_byte(dev, 0x01); // Enable command
    delayMS(10);

    // Set VRH voltage (adjusts brightness)
    ESP_LOGI(TAG, "Setting VRH");
    spi_master_write_command(dev, 0xC3); // VRH Set command
    spi_master_write_data_byte(dev, 0x12); // Voltage setting
    delayMS(10);

    // Set VDV voltage (fine-tunes brightness)
    ESP_LOGI(TAG, "Setting VDV");
    spi_master_write_command(dev, 0xC4); // VDV Set command
    spi_master_write_data_byte(dev, 0x20); // Voltage setting
    delayMS(10);

    // Set the frame rate to 60Hz
    ESP_LOGI(TAG, "Setting Frame Rate Control");
    spi_master_write_command(dev, 0xC6); // Frame Rate Control command
    spi_master_write_data_byte(dev, 0x0F); // Frame rate setting
    delayMS(10);

    // Configure power settings
    ESP_LOGI(TAG, "Setting Power Control 1");
    spi_master_write_command(dev, 0xD0); // Power Control 1 command
    spi_master_write_data_byte(dev, 0xA4); // Default setting
    spi_master_write_data_byte(dev, 0xA1); // Default setting
    delayMS(10);

    // Set positive voltage gamma control for color adjustment
    ESP_LOGI(TAG, "Setting Positive Voltage Gamma Control");
    spi_master_write_command(dev, 0xE0); // Positive Gamma Control command
    // Write gamma curve settings
    uint8_t positive_gamma[] = {0xD0, 0x08, 0x11, 0x08, 0x0C, 0x15, 0x39, 0x33,
    ...
    // Set negative voltage gamma control for color adjustment
    ESP_LOGI(TAG, "Setting Negative Voltage Gamma Control");
    spi_master_write_command(dev, 0xE1); // Negative Gamma Control command
    // Write gamma curve settings
    uint8_t negative_gamma[] = {0xD0, 0x08, 0x10, 0x08, 0x06, 0x06, 0x39, 0x44,
    ...
    // Enable display inversion for better color reproduction
    ESP_LOGI(TAG, "Enabling Display Inversion");
    spi_master_write_command(dev, 0x21); // Display Inversion On command
    delayMS(10);

    // Set the display to normal mode
    ESP_LOGI(TAG, "Setting Normal Display Mode");
    spi_master_write_command(dev, 0x13); // Normal Display Mode On command
    delayMS(10);

    // Turn on the display
    ESP_LOGI(TAG, "Turning Display On");
    spi_master_write_command(dev, 0x29); // Display On command
    delayMS(120); // Wait for the display to fully turn on

    // Turn on the backlight if the backlight pin is used
    if (dev->_bl >= 0) {
        gpio_set_level(dev->_bl, 1); // Set backlight pin high to turn on backlight
